...
export default function Profile() {
    ...
    const handleImportData = async () => {
        ...
        input.onchange = async (e) => {
            ...
            try {
                ...
                if (data.groceryList) localStorage.setItem("grocery:list:v2", JSON.stringify(data.groceryList));
                if (data.familyMembers) localStorage.setItem("family:members:v1", JSON.stringify(data.familyMembers));
                if (data.mealLogs) localStorage.setItem("family:meal:logs:v1", JSON.stringify(data.mealLogs));
                if (data.preferences) {
                    if (data.preferences.unitSystem) localStorage.setItem("unitSystem", data.preferences.unitSystem);
                    ...
                    if (data.preferences.intolerances) localStorage.setItem("filters:intolerances", data.preferences.intolerances);
                    if (data.preferences.maxTime) localStorage.setItem("filters:maxTime", data.preferences.maxTime);
                    if (data.preferences.mealType) localStorage.setItem("filters:mealType", data.preferences.mealType);
                    if (data.preferences.maxCalories) localStorage.setItem("filters:maxCalories", data.preferences.maxCalories);
                    if (data.preferences.healthScore) localStorage.setItem("filters:healthScore", data.preferences.healthScore);
                    if (data.preferences.pantry) localStorage.setItem("filters:pantry", JSON.stringify(data.preferences.pantry));
                }
                if (data.ratings) {
                    Object.keys(data.ratings).forEach((recipeId) => {
                        localStorage.setItem(`recipeRating:${recipeId}`, JSON.stringify(data.ratings[recipeId]));
                    });
                }
    ...
    const handleDeleteAccount = async () => {
        if (!confirm("Are you sure you want to delete your account? This will delete all your data and cannot be undone.")) {
            return;
        }

        if (!confirm("This is your last chance. All your favorites, meal plans, grocery lists, and family data will be permanently deleted. Continue?")) {
            return;
        }
        ...
        try {
            ...
            const keysToDelete = [
                "favorites", "meal:plan:v2", "grocery:list:v2", "unitSystem", "theme",
                "filters:diet", "filters:intolerances", "filters:maxTime", "filters:mealType",
                "filters:maxCalories", "filters:healthScore", "filters:pantry",
                "family:members:v1", "family:meal:logs:v1", "notifications:enabled"
            ];
                            ...
                            <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
                                ...
                                <div className="flex items-start gap-4 mb-4">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="mb-2">
                                            <label className="text-sm font-medium text-slate-600 dark:text-slate-400">Signed in with</label>
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-500 dark:text-slate-500 font-mono break-all">{user.id}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    >
                                        ...
                                        <div className="text-sm text-slate-600 dark:text-slate-400">Manage family members</div>
                                    </button>
                                </div>
                            </div>
                        ...
                        >
                            ...
                            <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
                                ...
                                <div className="space-y-4">
                                    <div>
                                        <label className="block text-sm font-medium mb-2">Max Cooking Time (minutes)</label>
                                        <input
                                            type="number"
                                            value={maxTime}
                        ...
                        >
                            ...
                            <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
                                ...
                                {pantry.length > 0 ? (
                                    ...
                                    <p className="text-sm text-slate-500">No pantry items yet. Add ingredients to find recipes using what you have!</p>
                                )}
                            </div>
                        </motion.div>
                    )}
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-slate-600 dark:text-slate-400">
                                            A modern recipe discovery app with meal planning, grocery lists, and family management features.
                                        </p>
                                    </div>
                                </div>
                            </div>
