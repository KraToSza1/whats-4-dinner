...
function RecipeCard({ recipe, onFavorite, isFavorite, index = 0 }) {
  ...
  if (import.meta.env.DEV) {
    ...
    // Only log if it's been more than 1 second since last log for this card
    if (!lastLog || now - parseInt(lastLog) > 1000) {
      console.log('🖼️ [RECIPE CARD] Component render', {
        id: recipe?.id,
        title: recipe?.title,
        index,
        isFavorite,
        hasImage: !!(recipe?.hero_image_url || recipe?.image),
      });
  ...
  const handleOpen = () => {
    console.log('🖼️ [RECIPE CARD] Card clicked, navigating to recipe', { id, title, index });
    if (!id) {
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden">
        <RecipeImageZoom isHovered={isHovered}>
          <img
            ...
            onLoad={e => {
              console.log('🖼️ [RECIPE CARD] Image loaded successfully', {
                id,
                src: e.currentTarget.src,
                title,
            ...
            onLoadStart={() => {
              console.log('🖼️ [RECIPE CARD] Image loading started', { id, title });
            }}
        ...
        <motion.button
          ...
          tabIndex={0}
          className="absolute top-2 right-2 z-10 inline-flex items-center justify-center w-8 h-8 xs:w-9 xs:h-9 sm:w-10 sm:h-10 rounded-full bg-white/95 dark:bg-slate-900/90 shadow-lg border-2 border-slate-200 dark:border-slate-700 backdrop-blur-sm transition-all touch-manipulation"
        ...
        >
          <motion.span
            ...
            aria-hidden
            className={`text-lg xs:text-xl sm:text-2xl ${isFavorite ? 'text-rose-500' : 'text-slate-400'}`}
          >
            {isFavorite ? '❤️' : '🤍'}
          </motion.span>
          {isFavorite && (
      ...
      <div className="p-2 xs:p-3 sm:p-4">
        <h3 className="text-xs xs:text-sm sm:text-base font-semibold line-clamp-2 leading-tight">
          {title}
        </h3>
      </div>
    </motion.div>
